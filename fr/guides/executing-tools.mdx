...
<Step title="Découvrir les Outils Disponibles">
...
<Step title="Exécuter l'Outil">
  Maintenant, nous allons démarrer l'outil en effectuant une requête `POST` vers le point de terminaison `/tools/{tool_id}/execute`. Vous aurez besoin de l'`id` de l'étape précédente. Dans le corps de la requête, nous fournirons les `inputs` requis par l'outil.
...
<Step title="Interroger les Résultats">
  Comme l'outil s'exécute de manière asynchrone, nous devons interroger la `details_url` pour vérifier le résultat final. Vous devrez vérifier ce point de terminaison périodiquement jusqu'à ce que le `status` passe à `completed` ou `failed`.
...
Vous savez maintenant comment découvrir et exécuter des outils puissants et pré-construits pour automatiser des flux de travail complexes avec l'API UBIK.
