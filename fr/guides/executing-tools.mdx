...
<Step title="Exécuter l'Outil">
  ...
  <Tip>
    **Isolation des Utilisateurs**: Si vous exécutez des outils pour le compte de vos utilisateurs finaux, vous pouvez isoler leurs données en passant l'en-tête `X-End-User-ID`.
    
    `curl -H "X-End-User-ID: user_123" ...`
    
    Pour plus de détails sur l'isolation des données et le multi-tenancy, voir le [Guide Authentification & Sécurité](/fr/guides/authentication).
  </Tip>
