...
> **Note** : Ces balises représentent l'*historique* de l'exécution de l'outil. Pour afficher la progression en temps réel (comme un indicateur de chargement ou des journaux) *pendant* l'exécution de l'outil, vous devez écouter les événements SSE décrits dans le guide **[Gestion du Streaming d'Outils](/fr/guides/streaming-results)**.
...
1.  **Début** : `<<TOOL_STEP_START/nom_outil:id_execution>>`
    ...
    *   **Temps réel** : Commencez à écouter les événements `tool_update` correspondant à cet `id_execution` (voir [Gestion du Streaming d'Outils](/fr/guides/streaming-results)).
...
Pour un tutoriel complet sur la combinaison de ces flux pour construire une interface riche, veuillez consulter le guide **[Gestion du Streaming d'Outils](/fr/guides/streaming-results)**.
