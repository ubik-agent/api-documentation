...
> **Note** : Certaines parties du backend peuvent émettre `agent_response_chunk`. L'implémentation recommandée attend spécifiquement `response_chunk`.
...
Les événements `agent_response_chunk` transportent le texte brut généré par l'agent. Ce texte contient souvent des balises structurées (comme `<<TOOL_STEP_START>>`, `<<thinking>>`, etc.) qui représentent l'état interne et les actions de l'agent.
