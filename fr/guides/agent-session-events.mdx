...
Événements liés à l'utilisation d'outils par l'agent. Notez que la *représentation textuelle* des appels d'outils (entrées/sorties) apparaît dans le flux `response_chunk`, tandis que ces événements fournissent des mises à jour de statut structurées.
...
<Tip>
  **Intégration UI Simplifiée** : Si vous préférez ne pas implémenter une interface utilisateur personnalisée pour les événements d'exécution d'outils, vous pouvez extraire le `tool_execution_id` de l'événement `tool_update` (lorsque le statut est `started`) et utiliser l'iframe **[Intégration d'Exécution d'Outil](/fr/guides/embedding-tools#integrer-une-execution-specifique)**. Cela vous permet d'afficher la progression et le résultat de l'outil avec une seule ligne de code.
</Tip>
...
Indique qu'une demande d'arrêt a été reçue pour la session. L'agent s'arrêtera gracieusement après avoir terminé les appels d'outils en cours (le cas échéant).
