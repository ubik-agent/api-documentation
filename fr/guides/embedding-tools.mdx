...
<Step title="Afficher l'iframe">
  Une fois que votre frontend dispose des informations d'intégration, vous pouvez construire l'URL de l'`<iframe>` et l'afficher. Le `access_token` du point de terminaison `embed-info` doit être passé comme paramètre de requête `jwt` à l'URL source de l'`iframe`.
  ...
  <Note>
    **Utiliser une clé API directe :** Pour les scénarios où vous préférez ne pas utiliser le point de terminaison `/embed-info` (par exemple, le rendu côté serveur), vous pouvez également vous authentifier en passant votre clé API UBIK directement en tant que paramètre de requête `apiKey`.
...
<Step title="Personnaliser l'Outil Intégré">
  ...
  - **`jwt`** (string) : Un JSON Web Token de courte durée obtenu depuis le point de terminaison `/tools/{tool_id}/embed-info`. C'est la méthode recommandée pour l'authentification côté client.
